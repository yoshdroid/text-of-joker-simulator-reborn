    ...
    *,
    ...
    for card_instance_id in list(state.players[player_id].trigger_zone.cards):
        ...
        if not _card_can_activate(state, player_id, card_instance_id):
            continue
        if not any(_ability_matches_window(state, ability, window_prefix, window_name, cause_event, player_id) for ability in card.abilities):
            continue
...
def _activate_card(
    ...
    *,
    ...
    matching_abilities = [
        ...
        if _ability_matches_window(state, ability, prefix, str(window_name), cause_event, player_id)
    ]
...
def _list_intercept_actions(state: GameState, player_id: str, window: str, cause_event: FactEvent) -> list[dict[str, Any]]:
    ...
    for card_instance_id in state.players[player_id].trigger_zone.cards:
        ...
        if not _card_can_activate(state, player_id, card_instance_id):
            continue
        if not any(_ability_matches_window(state, ability, "INTERCEPT", window, cause_event, player_id) for ability in card.abilities):
            continue
        actions.append(
            {
    ...
    for player in state.players.values():
        for card_instance_id in player.trigger_zone.cards:
            ...
            if not _card_can_activate(state, player.player_id, card_instance_id):
                continue
            if any(_ability_matches_window(state, ability, window_prefix, window_name, cause_event, player.player_id) for ability in card.abilities):
                return True
    return False
