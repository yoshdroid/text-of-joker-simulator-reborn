...
INTERCEPT_PRIORITY = {
    "1-0-085": 100,  # タイタンの鉄鎖
    "1-0-065": 92,   # パワーショーテージ
    "1-0-083": 86,   # フォトン
}

LOW_COST_UNIT_NOS = {"1-0-014", "1-0-015", "1-0-016", "1-0-017", "1-0-018", "1-0-022"}
PASS_ACTION_TYPES = {"pass", "no_block", "pass_window"}


class CodexYellowPlayer:
    def __init__(self, *, max_mulligans: int = 1) -> None:
    ...
    def choose_mulligan(self, message: dict[str, Any]) -> bool:
        ...
        has_low_unit = any(_card_no(card) in LOW_COST_UNIT_NOS for card in hand if isinstance(card, dict))
        has_too_many_non_units = sum(1 for card in hand if isinstance(card, dict) and card.get("category") != "unit") >= 4
        do_mulligan = not has_low_unit or has_too_many_non_units
        if do_mulligan:
    ...
    def _choose_block(self, legal_actions: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        if favorable:
            return max(favorable, key=lambda action: _unit_bp(action.get("unit")))
        return None
    ...
    def _choose_set_trigger(self, legal_actions: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        if not set_actions:
            return None
        return max(set_actions, key=lambda action: SET_PRIORITY.get(_action_card_no(action), 0))
    ...
    def _choose_drive(self, legal_actions: list[dict[str, Any]], message: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not drive_actions:
            return None
        own_battlefield = _own_battlefield(message)
        if len(own_battlefield) >= 4:
    ...
    def _choose_attack(self, legal_actions: list[dict[str, Any]]) -> dict[str, Any] | None:
        ...
        if not attacks:
            return None
        return max(attacks, key=lambda action: _unit_bp(action.get("unit")))
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Codex yellow deck JSON Lines player.")
    parser.add_argument("--max-mulligans", type=int, default=1)
    args = parser.parse_args()
    player = CodexYellowPlayer(max_mulligans=args.max_mulligans)
