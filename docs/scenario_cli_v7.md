...
| `barbatos_base_bp` | バルバトス | `1-0-051` バルバトスを進化ドライブし、対戦相手ユニットの基礎BPが永続で-4000される。ギガマムートがハッパロイドとの戦闘に勝利してLV2になっても BP は 4000 のままで、P2ターン終了後も戻らない。 | ✓ | ✓ | v8追加 |
| `battle_intercepts` | 悪の覚醒 / 不可侵防壁 | `battle_started` 後、攻撃側の `1-0-081` 悪の覚醒、ブロック側の `1-0-096` 不可侵防壁の順に発動し、BP修正が戦闘ダメージ前に入る。 | ✓ | ✓ | v8追加 |
...
| `ectoplasm_destroy` | エクトプラズム | 自分の唯一の青ユニット破壊時、発動契機ユニットが墓地へ移動する前に `1-0-092` エクトプラズムを発動し、対戦相手ユニットを1体破壊する。 | ✓ | ✓ | v8追加 |
...
| `new_armor_surprise_box_chain` | 新品の鎧 / サプライズボックス | trigger zone の `1-0-061` 新品の鎧が発動して左詰めされた後、隣の `1-0-057` サプライズボックスも続けて発動する。 | ✓ | ✓ | v8追加 |
