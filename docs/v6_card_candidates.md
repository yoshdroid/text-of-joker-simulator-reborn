...
| `1-0-007` | ゴライアス | OC 時の life damage、target なし SELF_OC |
...
| `1-0-026` | 戦神・毘沙門 | 自身以外全体破壊、PIG 順序、turn player first |
...
- 対象条件がカード属性、BP、種族、行動権、damage などに依存する場合の schema 表現。
    行動権消費の確認カードは 1-0-016 金色の狛犬 と 1-0-017 湖畔のアリエ とし、前者は OC 時、後者は CIP 時に相手の未行動ユニット1体を行動済みにする。
...
確認用カードは 1-0-019 ジャンプー とする。

必要な仕様:

- 戻されたユニットは battlefield から取り除き、破壊ではないため PIG は発動しない。
...
- discard pile からの移動は card instance を維持するか、新しい instance として扱うか。
    ...
    PIG の確認用カードは 1-0-028 スカルウォーカー とし、破壊時に同じ効果を解決する。
    スカルウォーカー自身の `SELF_PIG` 解決時点では、スカルウォーカーのカードはまだ battlefield にあるため、捨札回収候補には自身を含めない。
    選択式の確認用カードは 1-0-031 見習い魔導士リーナ とし、OC 時に自分の捨札からカードを1枚選んで手札に加える。
...
カードプールの unit BP は 3/4/5 のような千単位表記を許容し、ゲーム中は 3000/4000/5000 として扱う。
BP modifier も互換のため `-3` のような千単位表記を受け付け、ゲーム中は `-3000` として扱う。
